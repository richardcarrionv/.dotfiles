...
slop_resize() {
	wmctrl -ia "$1"
  # after focusing the window
  # windows that are not "floatin" can't be resized with wmctrl
  # enable floating mode with i3 before resizing
...
generate_window_list() {
    ...
    # in i3 workspace "-1" is scratchpad so we hide the windows on that
    if [[ "$ws" == "-1" ]]; then
      continue
    fi
